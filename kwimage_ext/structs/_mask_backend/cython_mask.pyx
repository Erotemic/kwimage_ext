...
r"""
cd ~/code/kwimage_ext/kwimage_ext/structs/_mask_backend/
CPATH=$CPATH:$(python -c "import numpy as np; print(np.get_include())") cythonize -a -i ~/code/kwimage_ext/kwimage_ext/structs/_mask_backend/cython_mask.pyx

Why is this compile command not working for debugging?

( cd ~/code/kwimage_ext/kwimage_ext/structs/_mask_backend && \
  CFLAGS="-I$PWD ${CFLAGS}" \
  CPATH="$(python -c 'import numpy as np; print(np.get_include())'):${CPATH}" \
  cythonize --3str -a -f -i cython_mask.pyx maskApi.c maskApi.h )
...
def encode(cnp.ndarray[cnp.uint8_t, ndim=3, mode='fortran'] mask):
...
def area(rleObjs):
    ...
    cdef cnp.npy_intp shape[1]
    shape[0] = <cnp.npy_intp> Rs._n
    a = np.array((Rs._n, ), dtype=np.uint8)
    a = cnp.PyArray_SimpleNewFromData(1, shape, cnp.NPY_UINT32, _a)
    PyArray_ENABLEFLAGS(a, cnp.NPY_ARRAY_OWNDATA)
    return a
...
def iou( dt, gt, pyiscrowd ):
    def _preproc(objs):
        ...
        return objs
    def _rleIou(RLEs dt, RLEs gt, cnp.ndarray[cnp.uint8_t, ndim=1] iscrowd, siz m, siz n, cnp.ndarray[cnp.double_t,  ndim=1] _iou):
        rleIou( <RLE*> dt._R, <RLE*> gt._R, m, n, <byte*> iscrowd.data, <double*> _iou.data )
    def _bbIou(cnp.ndarray[cnp.double_t, ndim=2] dt, cnp.ndarray[cnp.double_t, ndim=2] gt, cnp.ndarray[cnp.uint8_t, ndim=1] iscrowd, siz m, siz n, cnp.ndarray[cnp.double_t, ndim=1] _iou):
    ...
    def _len(obj):
        ...
        if type(obj) == RLEs:
            ...
            pass
        elif type(obj) == cnp.ndarray:
            N = obj.shape[0]
        return N
    # convert iscrowd to numpy array
    cdef cnp.ndarray[cnp.uint8_t, ndim=1] iscrowd = np.array(pyiscrowd, dtype=np.uint8)
    ...
    shape[0] = <cnp.npy_intp> m*n
    iou = cnp.PyArray_SimpleNewFromData(1, shape, cnp.NPY_DOUBLE, _iou)
    PyArray_ENABLEFLAGS(iou, cnp.NPY_ARRAY_OWNDATA)
...
def toBbox( rleObjs ):
    ...
    cdef cnp.npy_intp shape[1]
    shape[0] = <cnp.npy_intp> 4*n
    bb = np.array((1,4*n), dtype=np.double)
    bb = cnp.PyArray_SimpleNewFromData(1, shape, cnp.NPY_DOUBLE, _bb).reshape((n, 4))
    PyArray_ENABLEFLAGS(bb, cnp.NPY_ARRAY_OWNDATA)
    return bb

def frBbox(cnp.ndarray[cnp.double_t, ndim=2] bb, siz h, siz w ):
...
def frPoly( poly, siz h, siz w ):
    cdef cnp.ndarray[cnp.double_t, ndim=1] np_poly
    n = len(poly)
    Rs = RLEs(n)
...
def frUncompressedRLE(ucRles, siz h, siz w):
    cdef cnp.ndarray[cnp.uint32_t, ndim=1] cnts
    cdef RLE R
    cdef uint *data
    n = len(ucRles)
    objs = []
...
def _rle_bytes_to_array(counts_str):
    ...
    cdef cnp.ndarray[uint, ndim=1] new_counts = np.empty(num_chars, dtype=np.uint32)
    # create pure-c view
...
@cython.cdivision(True)
@cython.wraparound(False)
def _rle_array_to_bytes(cnp.ndarray[cnp.uint32_t, ndim=1] counts_arr):
    """ 
